...
async function example1_basicUsage() {
    ...
    // 3. 获取账号画像
    const profileManager = new ProfileManager(db, process.env.OPENAI_API_KEY, 'rabbitrun_eth');
...
async function example4_scenarios() {
    ...
    const sources = new Set(diverse.map((c) => c.content.source));
    ...
    if (quality.length > 0) {
        const avgScore = quality.reduce((sum, c) => sum + (c.aiScore || 0), 0) / quality.length;
...
export { example1_basicUsage, example2_customOptions, example3_saveRecommendations, example4_scenarios, example5_errorHandling, };
