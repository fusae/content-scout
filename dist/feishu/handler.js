...
export class CardActionHandler {
     ...
     * 处理复制草稿
     */
    async handleCopyDraft(openId, action) {
        ...
        // 6. 发送确认消息（包含可复制的草稿）
        await this.larkClient.sendText(openId, `✅ 草稿 ${draft_index + 1} 已准备好！\n\n${finalDraft}\n\n复制上面的内容，去 X 发布吧！`);
        logger.info(`Draft ${draft_index} copied for recommendation ${recommendation_id}`);
    }
    /**
     * 处理拒绝推荐
     */
    async handleReject(openId, action) {
        ...
        // 3. 发送确认消息
        await this.larkClient.sendText(openId, '👌 已记录你的反馈！\n\n我们会根据你的偏好优化后续推荐，避免类似内容。');
        logger.info(`Recommendation ${recommendation_id} rejected`);
    }
    /**
     * 发送错误消息
     */
    async sendErrorMessage(openId) {
        try {
            await this.larkClient.sendText(openId, '❌ 操作失败，请稍后重试或联系管理员。');
        }
        catch (error) {
