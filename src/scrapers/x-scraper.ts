...
export class XScraper extends BaseScraper {
   ...
   */
  private async searchTweets(bearerToken: string): Promise<XTweet[]> {
    const query = encodeURIComponent('(AI OR "AI agent" OR Codex OR LLM OR developer tools) lang:en -is:retweet');
    const url =
      `${this.baseUrl}/tweets/search/recent?query=${query}` +
      `&max_results=${this.maxTweets}` +
      '&tweet.fields=created_at,public_metrics,author_id' +
      '&expansions=author_id' +
      '&user.fields=username,name';

    const response = await this.axiosInstance.get<XSearchResponse>(url, {
      headers: {
    ...
    const users = new Map(
      (response.data.includes?.users || []).map((user) => [user.id, user])
    );
