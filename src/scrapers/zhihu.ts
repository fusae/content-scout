...
export class ZhihuScraper extends BaseScraper {
  ...
  protected baseUrl = 'https://www.zhihu.com/hot';
  ...
  async scrape(): Promise<ContentItem[]> {
    try {
      ...
      logger.error('Zhihu Hot scrape failed:', error as Error);
      if (this.browser) {
        await this.browser.close();
        this.browser = null;
      }
   ...
   * 解析知乎热榜
   */
  private async parseHotItems(page: Page): Promise<ZhihuHotItem[]> {
    try {
      // 等待内容加载
      ...
      logger.error('Failed to parse Zhihu hot items:', error as Error);
      return [];
    }
  }
   ...
   * 转换为标准格式
   */
  private convertToContentItem(item: ZhihuHotItem): ContentItem {
