...
### 2. 初始画像数据
- **文件**: `src/data/initial-profile.json`
- **内容**: 存储 Grok 分析的 @example_creator 账号画像数据
...
### 核心模块
- `src/profile/index.ts` - ProfileManager
- `src/profile/vectorizer.ts` - 向量化模块
- `src/profile/types.ts` - 类型定义

### AI 客户端
- `src/ai/embedding.ts` - OpenAI Embedding
- `src/ai/deepseek.ts` - DeepSeek 客户端

### 数据和配置
- `src/data/initial-profile.json` - 初始画像数据
